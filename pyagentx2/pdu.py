...
class NullHandler(logging.Handler):
    def emit(self, record):
        pass
logger = logging.getLogger('pyagentx2.pdu')
...
class PDU(object):
    ...
    def __init__(self, type=0):
        ...
        self.packet_id = 0
        self.error = pyagentx2.ERROR_NOAGENTXERROR
        self.error_index = 0
        self.decode_buf = ''
        self.state = {}
        self.values = []
    ...
    def dump(self):
        name = pyagentx2.PDU_TYPE_NAME[self.type]
    ...
    def encode_value(self, type, name, value):
        ...
        if type in [pyagentx2.TYPE_INTEGER]:
            buf += struct.pack('!l', value)
        elif type in [pyagentx2.TYPE_COUNTER32, pyagentx2.TYPE_GAUGE32, pyagentx2.TYPE_TIMETICKS]:
            buf += struct.pack('!L', value)
        elif type in [pyagentx2.TYPE_COUNTER64]:
    ...
    def encode(self):
        ...
        elif self.type == pyagentx2.AGENTX_PING_PDU:
            # No extra data
            pass

        elif self.type == pyagentx2.AGENTX_REGISTER_PDU:
            range_subid = 0
            timeout = 5
            priority = 127
        ...
        elif self.type == pyagentx2.AGENTX_RESPONSE_PDU:
    ...
    def decode_value(self):
        ...
        if vtype in [pyagentx2.TYPE_INTEGER, pyagentx2.TYPE_COUNTER32, pyagentx2.TYPE_GAUGE32, pyagentx2.TYPE_TIMETICKS]:
            ...
            data = data[0]
            self.decode_buf = self.decode_buf[4:]
        elif vtype in [pyagentx2.TYPE_COUNTER64]:
    ...
    def decode_header(self):
        try:
            ...
            ret = {
                'version': t[0],
                'pdu_type':t[1],
                'pdu_type_name':  pyagentx2.PDU_TYPE_NAME[t[1]],
                'flags':t[2],
                'reserved':t[3],
                'session_id':t[4],
                'transaction_id':t[5],
    ...
    def decode(self, buf):
        ...
        if ret['pdu_type'] == pyagentx2.AGENTX_RESPONSE_PDU:
            ...
            self.response = {
                'sysUpTime': t[0],
                'error':t[1],
                'error_name':pyagentx2.ERROR_NAMES[t[1]],
                'index':t[2],
            }
            # Decode VarBindList
            self.values = []
        ...
        elif ret['pdu_type'] == pyagentx2.AGENTX_GET_PDU:
            self.range_list = self.decode_search_range_list()

        elif ret['pdu_type'] == pyagentx2.AGENTX_GETNEXT_PDU:
            self.range_list = self.decode_search_range_list()

        elif ret['pdu_type'] == pyagentx2.AGENTX_TESTSET_PDU:
            # Decode VarBindList
            self.values = []
            while len(self.decode_buf):
                self.values.append(self.decode_value())
        elif ret['pdu_type'] in [pyagentx2.AGENTX_COMMITSET_PDU,
                                 pyagentx2.AGENTX_UNDOSET_PDU,
                                 pyagentx2.AGENTX_CLEANUPSET_PDU]:
            pass
        else:
            pdu_type_str = pyagentx2.PDU_TYPE_NAME.get(ret['pdu_type'], 'Unknown:'+ str(ret['pdu_type']))
